...
import { createOrderSchema, updateOrderSchema, latestEntryNoQuerySchema } from './orders.validators.js';
...
ordersRoutes.post('/', async (c) => {
  ...
  try {
    ...
    const data = createOrderSchema.parse({ ...body, storeId });
...
ordersRoutes.get('/latest-entry-no', async (c) => {
  const auth = getAuth(c);
  if (!auth) {
    return errorResponse(c, 401, 'Unauthorized', 'Not authenticated');
  }
  ...
  try {
    const latestEntryNo = await ordersService.getLatestEntryNo(c.env, storeId);
    return successResponse(c, { latestEntryNo });
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Failed to fetch latest entry number';
